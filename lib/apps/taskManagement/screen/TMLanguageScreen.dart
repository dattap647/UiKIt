          ...
          children: [
            AppTextField(
              ...
              decoration: TMInputDecoration(context,
                  hintText: "Search", borderColor: context.dividerColor),
              onChanged: (value) {
                languageSearchList = languageList
                    .where(
                      (element) => element.title!
                          .contains(value.capitalizeFirstLetter()),
                    )
                    .toList();
                if (languageListController.text.isNotEmpty &&
                    languageSearchList!.length == 0) {}
                setState(() {});
              },
            ).paddingAll(16),
            ListView.builder(
              itemCount: languageListController.text.isNotEmpty
                  ? languageSearchList!.length
                  : languageList.length,
              shrinkWrap: true,
              itemBuilder: (BuildContext context, int index) {
                TMCategoryModel data1 = languageListController.text.isNotEmpty
                    ? languageSearchList![index]
                    : languageList[index];
                ...
                return SettingItemWidget(
                  ...
                  onTap: () {
                    setState(
                      () {
                        selectedIndex = index;
                        TMCategoryModel data =
                            TMCategoryModel(title: data1.title.validate());
                        finish(context, data);
                      },
                    );
                  },
